...
def _band_from(filter_value, ccd) -> str | None:
    """Resolve a canonical band from ``FILTER``, falling back to CCD index."""
...
MUSCAT2_SITE = "teide"  # Teide Observatory (TCS 1.52m), Tenerife
...
def find_frames(data_dir: Path, target: str | None = None) -> tuple[dict, dict, dict]:
    ...
    """
    darks: dict[str, list[str]] = {b: [] for b in BAND_ORDER}
    flats: dict[str, list[str]] = {b: [] for b in BAND_ORDER}
    sciences: dict[str, list[str]] = {b: [] for b in BAND_ORDER}

    def _bucket(band: str | None, obj: str, path: str) -> None:
        if band is None:
            return
        if obj == "DARK":
            darks.setdefault(band, []).append(path)
        elif obj == "FLAT":
            flats.setdefault(band, []).append(path)
...
def calibrate_band(
    ...
    if solve_wcs == "twirl":
        ...
        seq = SequenceParallel(
            [calib, SaveCalibratedFITS(output_dir, wcs=wcs, wcs_method="twirl", site=MUSCAT2_SITE)],
        ...
        seq = SequenceParallel(
            [calib, SaveCalibratedFITS(output_dir, wcs_method="astrometry.net", site=MUSCAT2_SITE)],
            name=f"[{band}] calibrating",
        )
        seq.run(sciences)
